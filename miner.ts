...
function createCoinbase(
    ...
    if (config.segwit) {
        ...
        commits.push(
            Buffer.concat([
                wCommitHeader,
                bitcoin.crypto.hash256(Buffer.concat([merkleRoot(wtxids), Buffer.alloc(32)])),
            ]),
        );
    ...
    while (true) {
        ...
        const scriptSig = bitcoin.script.compile([
            bitcoin.script.signature.encode(
                ECPair.fromWIF(await btc('dumpprivkey', 'tb1qllllllxl536racn7h9pew8gae7tyu7d58tgkr3'), network).sign(
                    sighash,
                ),
